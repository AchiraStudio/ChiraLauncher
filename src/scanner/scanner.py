...
CODEX_SAVE_BASE    = expand(r"%SYSTEMDRIVE%\Users\Public\Documents\Steam\CODEX")
GOLDBERG_SAVE_BASE = expand(r"%APPDATA%\Goldberg SteamEmu Saves")
GOLDBERG_SAVE_BASE_ALT = expand(r"%APPDATA%\GSE Saves")
ANADIUS_SAVE_BASE  = expand(r"%LOCALAPPDATA%\anadius\LSX emu")


def find_codex_achievements(app_id: str) -> Dict[str, Any]:
    save_dir = CODEX_SAVE_BASE / app_id
    result: Dict[str, Any] = {"save_folder": None, "achievements_ini": None, "achievements_json": None}
    if save_dir.is_dir():
        result["save_folder"] = str(save_dir)
        ini  = save_dir / "achievements.ini"
        jsn  = save_dir / "achievements.json"
        result["achievements_ini"]  = str(ini) if ini.is_file() else None
        result["achievements_json"] = str(jsn) if jsn.is_file() else None
    return result


def find_goldberg_achievements(app_id: str) -> Dict[str, Optional[str]]:
    result: Dict[str, Optional[str]] = {"save_folder": None, "achievements_json": None}

    # Try primary Goldberg path
    save_dir = GOLDBERG_SAVE_BASE / app_id
    if not save_dir.is_dir():
        # Fallback to GSE Saves
        save_dir = GOLDBERG_SAVE_BASE_ALT / app_id
...
def find_anadius_achievements(content_id: str) -> Dict[str, Any]:
    ...
    # Find XML
    pattern = str(base / f"achievement-*_{content_id}_*.xml")
    matches = glob.glob(pattern)
    if not matches:
        # Fallback: any xml containing the content_id
        for p in base.glob("*.xml"):
            if content_id in p.name:
                matches.append(str(p))

    if matches:
        result["achievements_xml"] = matches[0]
        result["save_folder"] = str(base)

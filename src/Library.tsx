...
function LibraryListItem({
    ...
    const coverUrl = getValidImageUrl(game.cover_image_path || (game as any).cover_path);
...
export function Library() {
    ...
    // Reset errors and modals when active game changes
    useEffect(() => {
        setBgError(false);
    ...
    useEffect(() => {
        if (activeGame) {
            const bgUrl = activeGame.background_image_path || (activeGame as any).background_path || activeGame.cover_image_path;
            const valid = bgUrl ? getValidImageUrl(bgUrl) : null;
            setCurrentBg(valid);
        } else {
            setCurrentBg(null);
        }
    ...
    const coverUrl = activeGame ? getValidImageUrl(activeGame.cover_image_path || (activeGame as any).cover_path) : null;
    const bgUrl = activeGame ? getValidImageUrl(activeGame.background_image_path || (activeGame as any).background_path || activeGame.cover_image_path) : null;

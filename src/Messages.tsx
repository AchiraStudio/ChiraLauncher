...
import { Send, Lock, ChevronLeft, User, ShieldCheck, PlusCircle, Loader2 } from "lucide-react";
    ...
    return (
        <div className="absolute inset-0 flex flex-col bg-[#08090f] overflow-hidden">
            ...
            <div className="flex flex-col flex-1 min-h-0 max-w-5xl mx-auto w-full relative z-10 p-6 md:p-10">
                ...
                <div className="flex-1 overflow-y-auto space-y-6 mb-6 pr-2 custom-scrollbar flex flex-col min-h-0">
                    {messages.length === 0 ? (
                ...
                <div className="bg-[#1e2330] border border-white/10 rounded-2xl p-3 flex flex-col mt-auto shadow-2xl shrink-0">
                    {isGeneratingKeys ? (
                        ...
                        <form onSubmit={handleSend} className="flex items-center gap-4">
                            <div className="bg-white/5 border border-white/10 rounded-full p-3 text-white/30 hover:text-white hover:bg-white/10 cursor-pointer transition-all shrink-0">
                                <PlusCircle size={20} />
                            </div>
                            <input
                                type="text"
                            ...
                            />
                            <button type="submit" disabled={!input.trim() || isSending} className="p-3.5 shrink-0 bg-accent text-black rounded-full hover:brightness-110 transition-all disabled:opacity-50 shadow-[0_0_15px_rgba(var(--color-accent),0.3)] active:scale-95">

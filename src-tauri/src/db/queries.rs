...
pub fn insert_game_conn(conn: &Connection, game: NewGame) -> rusqlite::Result<usize> {
    ...
    let (
        ...
        restored_first,
        restored_last,
        restored_ach_unlocked,
        restored_ach_total,
    ): (i64, i64, Option<String>, Option<String>, i64, i64) = conn
        .query_row(
            "SELECT playtime_seconds, session_count, first_played, last_played,
                    achievements_unlocked, achievements_total
             FROM playtime_orphans
             WHERE (steam_app_id = ?1 AND ?1 IS NOT NULL) OR title_key = ?2
             ORDER BY playtime_seconds DESC LIMIT 1",
    ...
    conn.execute(
        "INSERT INTO games (
            ...
            crack_type, app_id,
            playtime_seconds, session_count, first_played, last_played,
            achievements_unlocked, achievements_total,
            detected_metadata_path, detected_earned_state_path
         )
         VALUES (
        ...
        params![
            ...
            game.source,
            game.added_at,
            game.installed_size.map(|x| x as i64),
            game.install_dir,
            game.publisher,
            game.release_date,
            game.genres,
            ...
            restored_ach_unlocked,
            restored_ach_total,
            game.detected_metadata_path,
            game.detected_earned_state_path,
        ],
    )
}
...
pub fn delete_game_conn(conn: &Connection, id: &str) -> rusqlite::Result<usize> {
    ...
    let _ = conn.execute(
        "INSERT INTO playtime_orphans (
         ...
         SELECT
             CAST(steam_app_id AS TEXT),
             LOWER(REPLACE(REPLACE(REPLACE(title, ' ', '_'), '-', '_'), '.', '_')),
             COALESCE(playtime_seconds, 0),
             COALESCE(session_count, 0),
             first_played,
             last_played,
             COALESCE(achievements_unlocked, 0),
             COALESCE(achievements_total, 0)
...
pub fn update_assets_conn(
    ...
    conn.execute(
        "UPDATE games SET cover_path = COALESCE(?1, cover_path), background_path = COALESCE(?2, background_path) WHERE id = ?3",
        params![cover_path, background_path, id],
    )
}

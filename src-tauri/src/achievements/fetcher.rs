...
pub async fn fetch_and_write_achievements(
    ...
    let mut download_tasks: Vec<Pin<Box<dyn Future<Output = Result<(), String>> + Send>>> =
        Vec::new();
    ...
    for ach in available {
        ...
        if !icon_url.is_empty() {
            ...
            download_tasks.push(Box::pin(async move {
                download_icon(&client_cl, &url_cl, &path_cl).await
            }));
        }
    ...
    let json_bytes =
        serde_json::to_vec_pretty(&goldberg_achievements).map_err(|e| e.to_string())?;

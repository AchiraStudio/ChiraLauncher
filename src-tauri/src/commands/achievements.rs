use crate::achievements::{
    sync_achievements, has_local_achievements, Achievement,
    discover_achievements, looks_like_metadata, 
    CrackType, SyncOptions,
};
...
pub async fn get_achievements(
    ...
    if let Some(install_dir) = game.install_dir {
        ...
        let crack_type: Option<CrackType> = game.crack_type
            .as_deref()
            .and_then(|s| serde_json::from_str(&format!("\"{}\"", s)).ok());

        if let Some(ref id) = db_app_id {
            crate::achievements::try_auto_generate(
                id,
                Path::new(&install_dir),
                crack_type.as_ref(),
                Some(&settings.steam_api_key),
            ).await;
        }
        ...
        let (achievements, _) = sync_achievements(
            &game_id, &install_dir,
            db_app_id.as_deref(),
            &opts,
        );
...
pub async fn sync_game_achievements(
    ...
    if let Some(install_dir) = game.install_dir {
        ...
        let (achievements, from_cache) = sync_achievements(
            &game_id, &install_dir,
            db_app_id.as_deref(),
            &opts,
        );
        Ok(SyncedAchievements { achievements, from_cache })
    } else {
        Ok(SyncedAchievements { achievements: vec![], from_cache: false })
    }
}
...
pub async fn check_local_achievements(
    ...
    Ok(game.install_dir
        .map(|dir| has_local_achievements(
            &dir,
...
/// Rich diagnostics — tells the user exactly what was found and where.
#[tauri::command]
pub async fn get_achievement_diagnostics(
    ...
    let _settings = crate::settings::get_settings(pool).unwrap_or_default();
    let db_app_id = game.steam_app_id.map(|id| id.to_string());
    ...
    // ── Metadata ──────────────────────────────────────────────────────────────
    let meta_valid = discovery.metadata_path.as_ref().map(|p| looks_like_metadata(p)).unwrap_or(false);

    let meta_count = discovery.metadata_path.as_ref()
        .and_then(|p| std::fs::read_to_string(p).ok())
        .and_then(|s: String| serde_json::from_str::<serde_json::Value>(&s).ok())
        .and_then(|v: serde_json::Value| v.as_array().map(|a| a.len()))
        .unwrap_or(0);

    let mut log = discovery.probe_log.clone();
    if let Some(ref p) = discovery.metadata_path {
        log.push(format!("Metadata: {} ({} achievements)", p.display(), meta_count));
    } else {
    ...
    let (earned_path_str, earned_format, earned_count) = if let Some(ref p) = discovery.save_path {
        ...
        log.push(format!("Save file: Found at {} ({} achievements unlocked)", p.display(), count));
        (
            Some(p.to_string_lossy().to_string()),
            Some(p.extension().and_then(|e| e.to_str()).unwrap_or("unknown").to_string()),
            count,
        )
    } else {
    ...
    Ok(AchievementDiagnostics {
        ...
        app_id,
        metadata_path:       discovery.metadata_path.map(|p| p.to_string_lossy().to_string()),
